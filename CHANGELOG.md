...
- Hook output now shows "Linting your last session..." immediately after session exit (before analysis runs) and waits for Enter to dismiss
- Hook output now wrapped in a visual box with session ID header and separator lines
- SessionEnd hook now writes output directly to `/dev/tty` via `--tty` flag, bypassing Claude Code's stdout suppression
- `ai-lint hook install` now auto-upgrades older hook formats instead of skipping with "already installed"

### Fixed
- Hook no longer triggers recursive `claude -p` calls — internal LLM calls now pass `--settings '{"disableAllHooks": true}'`
...
- Hook no longer spams "Your report is getting ready..." repeatedly in the terminal
